...
class LC_Icon_Box extends Atomic_Widget_Base {
    use Has_Template;

    public static function get_element_type(): string {
        return 'e-lc-icon-box';
    }
    ...
    protected static function define_props_schema(): array {
        return [
            ...
            'title' => Html_V3_Prop_Type::make()
                ->default( [
                    'content'  => String_Prop_Type::generate( __( 'Icon Box Title', 'lc-addons-kit-for-elementor' ) ),
                    'children' => [],
                ] )
                ->description( 'The title of the icon box.' ),

            'description' => String_Prop_Type::make()
                ->default( __( 'Lorem ipsum dolor sit amet, consectetur adipiscing elit.', 'lc-addons-kit-for-elementor' ) )
                ->description( 'The description of the icon box.' ),
    ...
    protected function define_atomic_controls(): array {
            ...
            Section::make()
                ...
                ->set_items( [
                    Inline_Editing_Control::bind_to( 'title' )
                        ...
                        ->set_placeholder( __( 'Icon Box Title', 'lc-addons-kit-for-elementor' ) ),
        ...
        // TODO(atomic): classic "icon picker", icon styling (color, background-color, border, radius, padding),
        // alignment, box styling (background, border, radius, shadow, padding) were styled via selectors.
    ...
    protected function get_templates(): array {
        return [
            'elementor/elements/lc-icon-box' => __DIR__ . '/lc-icon-box.html.twig',
        ];
    }
    ...
    public function render_markdown(): string {
        ...
        if ( ! empty( $title ) ) {
            ...
            $title_md = str_repeat( '#', $level ) . ' ' . $title;
            if ( ! empty( $settings['link']['href'] ) ) {
                $title_md = str_repeat( '#', $level ) . ' [' . $title . '](' . esc_url( $settings['link']['href'] ) . ')';
            }
            $md .= $title_md . "\n\n";
        }
        if ( ! empty( $desc ) ) {
