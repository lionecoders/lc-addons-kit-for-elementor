...
class LCAKE_Kit_Image_Accordion extends Widget_Base {
	...
	public function get_title() {
		return esc_html__( 'Image Accordion', 'lc-addons-kit-for-elementor' );
	}
	...
	public function get_keywords() {
		return array( 'image', 'accordion', 'gallery', 'hover', 'effect' );
	}

	public function get_style_depends() {
		return array( 'lcake-kit-image-accordion' );
	}
		...
		$this->start_controls_section(
			...
			$this->add_control(
				...
				array(
					...
					'default'     => array(
						array( 'lcake_img_accordion_title' => esc_html__( 'This is title', 'lc-addons-kit-for-elementor' ) ),
						array( 'lcake_img_accordion_icon' => esc_attr( 'icon icon-minus' ) ),
						array( 'lcake_img_accordion_link' => esc_url( '#' ) ),
						array( 'lcake_img_accordion_button_label' => esc_html__( 'Read More', 'lc-addons-kit-for-elementor' ) ),
					),
	...
	protected function render_raw() {
		...
		<div class="lcake-image-accordion lcake-image-accordion-wraper">
			<?php
			foreach ( $lcake_img_accordion_items as $key => $item ) :
				...
				if ( isset( $item['lcake_img_accordion_enable_wrap_link'] ) && $item['lcake_img_accordion_enable_wrap_link'] == 'yes' ) {
					$wrap_link        = $item['lcake_img_accordion_wrap_link_url'] ?? array();
				...
				<label <?php $this->print_render_attribute_string( 'wrap-link-' . $key ); ?>>
					<span class="lcake-accordion-content">
						<?php
						if ( $item['lcake_img_accordion_enable_pupup'] == 'yes' || $item['lcake_img_accordion_enable_project_link'] == 'yes' ) {
							...
							<?php
							if ( $item['lcake_img_accordion_enable_pupup'] == 'yes' ) {
								...
								?>
									...
									if ( $is_new || $migrated ) {
										...
										// new icon
										\Elementor\Icons_Manager::render_icon( $item['lcake_img_accordion_pup_up_icons'], array( 'aria-hidden' => 'true' ) );
									} else {
										?>
							...
							<?php } ?>
							<?php
							if ( $item['lcake_img_accordion_enable_project_link'] == 'yes' && ! empty( $item['lcake_img_accordion_project_link']['url'] ) ) {
								...
								?>
									...
									if ( $is_new || $migrated ) {
										...
										// new icon
										\Elementor\Icons_Manager::render_icon( $item['lcake_img_accordion_project_link_icons'], array( 'aria-hidden' => 'true' ) );
									} else {
										?>
								...
								<?php if ( $item['lcake_img_accordion_enable_icon'] == 'yes' ) : ?>
									<?php if ( $item['lcake_img_accordion_title_icon_position'] == 'left' ) : ?>
										...
										if ( $is_new || $migrated ) {
											...
											// new icon
											\Elementor\Icons_Manager::render_icon( $item['lcake_img_accordion_title_icons'], array( 'aria-hidden' => 'true' ) );
										} else {
											?>
							...
							<?php
							if ( $item['lcake_img_accordion_enable_button'] == 'yes' ) :
