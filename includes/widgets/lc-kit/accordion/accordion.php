...
class LCAKE_Kit_Accordion extends \Elementor\Widget_Base {
	...
	public function get_script_depends() {
		return array( 'lcake-kit-accordion' );
	}
	...
	public function get_keywords() {
		return array( 'accordion', 'tabs', 'toggle', 'collapsible', 'faq' );
	}
	...
	protected function add_style_controls() {
		...
		$this->start_controls_tab(
			...
			array( 'label' => esc_html__( 'CLOSED', 'lc-addons-kit-for-elementor' ) )
		);
		...
		$this->start_controls_tab(
			...
			array( 'label' => esc_html__( 'OPEN', 'lc-addons-kit-for-elementor' ) )
		);
		...
		foreach ( $settings['accordion_items'] as $index => $item ) {
			...
			if ( ! empty( $item['icon']['value'] ) || ! empty( $item['active_icon']['value'] ) ) {
				...
				if ( ! empty( $item['active_icon']['value'] ) ) {
					...
					\Elementor\Icons_Manager::render_icon( $item['active_icon'], array( 'aria-hidden' => 'true' ) );
					echo '</span>';
				}
				echo '</div>';
			}
