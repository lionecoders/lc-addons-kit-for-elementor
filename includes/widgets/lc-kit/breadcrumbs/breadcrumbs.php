...
class LCAKE_Kit_Breadcrumbs extends \Elementor\Widget_Base {
	...
	public function get_icon() {
		return 'eicon-share-arrow lcake-mveous-badge';
	}

	public function get_style_depends() {
		return array( 'lcake-kit-breadcrumbs-css' );
	}
	...
	private function get_trail( $settings ) {
		...
		if ( is_singular() ) {
			...
			$trail[] = array(
				'text'    => esc_html__( '404 Not Found', 'lc-addons-kit-for-elementor' ),
				'url'     => '',
				'current' => true,
			);
		} elseif ( is_home() || is_front_page() ) {
			$trail[] = array(
				'text'    => esc_html__( 'Blog', 'lc-addons-kit-for-elementor' ),
				'url'     => '',
				'current' => true,
			);
		} elseif ( is_archive() ) {
