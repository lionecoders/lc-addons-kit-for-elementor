...
class LC_Header_Footer_Site_Logo extends \Elementor\Widget_Base {
	...
	public function get_style_depends() {
		return array( 'lc-header-footer-site-logo-css' );
	}
		...
		$this->add_control(
			...
			array(
				...
				'description' => sprintf(
					...
					esc_html__( 'Defaults to your Customizer site logo if left empty. Set it %1$shere%2$s.', 'lc-addons-kit-for-elementor' ),
		...
		if ( empty( $logo_url ) ) {
			if ( current_user_can( 'customize' ) ) {
				echo '<p class="lc-hf-notice">' . sprintf(
					/* translators: %s: Link to WordPress Customizer */
					esc_html__( 'No logo set. Choose one above or set a Site Logo in the %1$sWordPress Customizer%2$s.', 'lc-addons-kit-for-elementor' ),
					'<a href="' . esc_url( admin_url( 'customize.php?autofocus[control]=custom_logo' ) ) . '" target="_blank">',
					'</a>'
				) . '</p>';
			}
